...
def main():
    ...
    Param2 = round(5*float(args.Parameter2)/1000000.0 + 25.0/1000000.0, 8)
    Param3 = round(float(args.Parameter3)/10.0, 3)
    ...
    right_bound = 4
    left_bound = -right_bound / Rg_ratio_UN_3_1
    ...
    for i in range(int(number_of_resets)):
        ...
        for j in range(int(steps_before_reset)):
            if (round(current_time, 2) % 100 == 0) and (current_time > 100000):
                x_list.append(x)
    ...
    dataframe = pd.DataFrame(data=dictionary, columns=["Times", "Positions", "RFs"])

    dataframe.to_csv("./U" + str(Param2) + "_S" + str(Param3) + "_" + str(SimNum) + ".csv")
