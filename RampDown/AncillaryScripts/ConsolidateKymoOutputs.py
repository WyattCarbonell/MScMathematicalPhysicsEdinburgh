       ...
       #Remove header
        ...
        while(x!= ''):
            ...
            if(x != ''):
                ...
                master_top_list[-1].append(topology)
...
for i in range(len(master_top_list)):
    if len(master_top_list[x]) < 2500:
        master_top_list.pop(x)
        x = x-1
    elif len(master_top_list[x]) < min_len:
        min_len = len(master_top_list[x])
    x = x+1

data_list = []
for i in range(min_len):
    total = 0
    count = len(master_top_list)
    for item in master_top_list:
        if item[i] == " UN ":
            total = total + 1

    data_list.append(total/count)
dictionary = {"Percents": data_list}
